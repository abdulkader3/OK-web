...
import { getBillById, BillDetail } from '@/src/services/bigBossService';
...
export default function BillDetailScreen() {
    ...
    const getBigBossName = (bigBossId: string | { _id: string; name: string; description?: string }) => {
        if (typeof bigBossId === 'object') {
            return bigBossId.name;
        }
        return 'Unknown';
    };

    const getCreatedByName = (createdBy: { _id: string; name: string; email: string } | undefined) => {
        if (createdBy) {
    ...
    const openAttachment = async () => {
        if (bill?.attachment?.url) {
            try {
                await Linking.openURL(bill.attachment.url);
            } catch (err) {
    ...
    const bigBossInfo = typeof bill.bigBossId === 'object' ? bill.bigBossId : null;
            ...
            <View style={styles.container}>
                ...
                >
                    <View style={styles.amountCard}>
                        ...
                        <Text style={styles.amountValue}>{formatCurrency(bill.amount)}</Text>
                    </View>

                    <View style={styles.section}>
                        <Text style={styles.sectionTitle}>Big Boss</Text>
                        <View style={styles.card}>
                            <View style={styles.staffRow}>
                                ...
                                <View style={styles.staffInfo}>
                                    <Text style={styles.staffName}>{getBigBossName(bill.bigBossId)}</Text>
                                    {bigBossInfo?.description && (
                                        <Text style={styles.staffEmail}>{bigBossInfo.description}</Text>
                                    )}
                                </View>
                            </View>
                        </View>
                        ...
                        <View style={styles.card}>
                            <View style={styles.detailRow}>
                                ...
                                <View style={styles.detailContent}>
                                    ...
                                    <Text style={styles.detailValue}>{getMonthName(bill.month)} {bill.year}</Text>
                                </View>
                            </View>
                            {bill.description && (
                                <>
                                ...
                                <>
                                    ...
                                    <View style={styles.detailRow}>
                                        ...
                                        <View style={styles.detailContent}>
                                            ...
                                            <Text style={styles.detailValue}>{formatDate(bill.updatedAt)}</Text>
                                        </View>
                                    </View>
                                </>
                            )}
                            {bill.createdBy && typeof bill.createdBy === 'object' && (
                                <>
                                    <View style={styles.divider} />
                                    <View style={styles.detailRow}>
                                        ...
                                        <View style={styles.detailContent}>
                                            ...
                                            <Text style={styles.detailValue}>{getCreatedByName(bill.createdBy)}</Text>
                                        </View>
                                    </View>
                                </>
                            )}
